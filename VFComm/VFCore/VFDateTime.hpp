...
#ifndef VF_DATETIME_HPP
#define VF_DATETIME_HPP
...
{
public:
	VFTimeSpan(int seconds)
	{
		Init(0, 0, 0, seconds, 0);
	}

	VFTimeSpan(int hours, int minutes, int seconds)
	{
		Init(0, hours, minutes, seconds, 0);
	}

	VFTimeSpan(int days, int hours, int minutes, int seconds)
	{
		Init(days, hours, minutes, seconds, 0);
	}

	VFTimeSpan(int days, int hours, int minutes, int seconds, int milliseconds)
	{
	...
	static VFTimeSpan FromMilliseconds(int milliseconds)
	{
		return VFTimeSpan(0, 0, 0, 0, milliseconds);
	}

	static VFTimeSpan FromSeconds(int seconds)
	{
		return VFTimeSpan(0, 0, 0, seconds, 0);
	}

	static VFTimeSpan FromMinutes(int minutes)
	{
		return VFTimeSpan(0, 0, minutes, 0, 0);
	}

	static VFTimeSpan FromHours(int hours)
	{
		return VFTimeSpan(0, hours, 0, 0, 0);
	}

	static VFTimeSpan FromDays(int days)
	{
		return VFTimeSpan(days, 0, 0, 0, 0);
	}
	...
	bool operator<(const VFTimeSpan &ts) const
	{
...
public:
	...
	VFDateTime(int day, int month, int year)
	{
		Init(day, month, year, 0, 0, 0, 0);
	}

	VFDateTime(int day, int month, int year, int hour, int minute, int second)
	{
		Init(day, month, year, hour, minute, second, 0);
	}

	VFDateTime(int day, int month, int year, int hour, int minute, int second, int millisecond)
	{
		Init(day, month, year, hour, minute, second, millisecond);
	}

	VFDateTime(time_t timestamp)
	{
	...
	std::string GetNameOfDay(int day)
	{
		...
		{
			throw VFException("Day %d is not in valid weekday range ( %d - %d )", day, MIN_WEEKDAY, MAX_WEEKDAY);
		}
	}
	...
	std::string GetNameOfMonth(int month)
	{
		...
		{
			throw VFException("Month %d is not in valid range ( %d - %d )", month, MIN_MONTH, MAX_MONTH);
		}
	}

	Day GetDayOfWeek()
	{
		return static_cast<VFDateTime::Day>(GetTMStruct().tm_wday);
	}
	...
	bool operator<(const VFDateTime &dt) const
	{
	...
	VFDateTime operator+(const VFTimeSpan &ts)
	{
		VFDateTime *tmp = this;
		tmp->Add(&ts);
		return *tmp;
	...
	void operator+=(const VFTimeSpan &ts)
	{
		Add(&ts);
	}
...
protected:
	...
	void Init(int day, int month, int year, int hour, int minute, int second, int millisecond)
	{
		...
		if (!IsValidMonth(month))
		{
			throw VFException("Month %d is not in range", month);
		}

		if (!IsValidDayOfMonth(day, month))
		{
			throw VFException("Day %d is not in month %d's range", day, month);
		}

		if (!IsValidHour(hour))
		{
			throw VFException("Hour %d is not in valid range ( %d - %d )", hour, MIN_HOUR, MAX_HOUR);
		}

		if (!IsValidMinute(minute))
		{
			throw VFException("Minute %d is not in valid range ( %d - %d )", minute, MIN_MINUTE, MAX_MINUTE);
		}

		if (!IsValidSecond(second))
		{
			throw VFException("Second %d is not in valid range ( %d - %d )", second, MIN_SECOND, MAX_SECOND);
		}

		if (!IsValidMillisecond(millisecond))
		{
			throw VFException("Millisecond %d is not in valid range ( %d - %d )", millisecond, MIN_MILLISECOND, MAX_MILLISECOND);
		}
